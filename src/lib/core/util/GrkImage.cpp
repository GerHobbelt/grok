  ...
  else
  {
    switch(decompressFormat)
    {
      case GRK_FMT_BMP:
        ...
        break;
      case GRK_FMT_PXM:
        packedRowBytes = grk::PlanarToInterleaved<int32_t>::getPackedBytes(ncmp, decompressWidth,
                                                                           prec > 8 ? 16 : 8);
        break;
      default:
        packedRowBytes =
...
bool GrkImage::compositeInterleaved(const Tile* src, uint32_t yBegin, uint32_t yEnd)
{
  ...
  iter->interleave(const_cast<int32_t**>(planes), src->numcomps_, interleavedData.data_ + destIndex,
...
bool GrkImage::compositePlanar(const GrkImage* src)
{
  for(uint16_t compno = 0; compno < src->numcomps; compno++)
  {
    ...
    {
      Logger::logger_.warn(
          "GrkImage::compositePlanar: cannot generate composite bounds for component %u", compno);
      continue;
    }
    auto destComp = comps + compno;
    if(!destComp->data)
