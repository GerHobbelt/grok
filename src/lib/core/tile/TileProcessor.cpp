...
bool TileProcessor::createWindowBuffers(const GrkImage* outputImage)
{
  for(uint16_t compno = 0; compno < tile->numcomps_; ++compno)
  {
    ...
    else
    {
      ...
      unreducedImageCompWindow = unreducedImageWindow.scaleDownCeil(imageComp->dx, imageComp->dy);
      ...
      for(uint8_t bandIndex = 0; bandIndex < res->numTileBandWindows; bandIndex++)
      {
        ...
        for(auto prc : band->precincts)
        {
          ...
          for(uint64_t cblkno = 0; cblkno < prc->getNumCblks(); cblkno++)
          {
            ...
            {
              compress_synch_with_plugin(this, compno, resno, bandIndex, prc->precinctIndex, cblkno,
                                         band, cblk, &numPix);
            }
            if(!single_lossless)
            {
        ...
        for(auto prc : band->precincts)
        {
          for(uint64_t cblkno = 0; cblkno < prc->getNumCblks(); cblkno++)
          {
            ...
            else
            {
              ...
              for(uint32_t passno = cblk->numPassesInPreviousPackets; passno < cblk->numPassesTotal;
                  passno++)
              {
                uint32_t dr;
                double dd;
