...
bool T1::alloc(uint32_t width, uint32_t height)
{
  if(width == 0 || height == 0)
  {
    Logger::logger_.error(
        "Unable to allocated memory for degenerate code block of dimensions %ux%u", width, height);
    return false;
  }
        ...
        {                                                                                         \
          ...
          if(type == T1_TYPE_RAW)                                                                 \
            mqc_bypass_enc_macro(mqc, c, ct, v) else mqc_encode_macro(                            \
                mqc, curctx, a, c, ct, v ^ getspb(lu)) update_flags(flagsp, ci, v, w + 2, vsc);   \
        }                                                                                         \
...
void T1::enc_sigpass(int32_t bpno, int32_t* nmsedec, uint8_t type, uint32_t cblksty)
{
  ...
  if(k < h)
  {
    for(i = 0; i < w; ++i)
    {
      ...
      {
        enc_sigpass_step_macro(pdata, 3 * (j - k), (j == k && (cblksty & GRK_CBLKSTY_VSC) != 0));
  ...
  for(k = 0; k < (h & ~3U); k += 4, flagsp += 2)
  {
    for(uint32_t i = 0; i < w; ++i, ++flagsp)
    {
      ...
      if(agg)
      {
        for(; runlen < 4; ++runlen)
        {
          if(smr_abs(uncompressedData[((k + runlen) * uncompressedDataStride) + i]) & (uint32_t)one)
            break;
        }
    ...
    {                                                                                              \
      ...
      {                                                                                            \
        ...
        {                                                                                          \
          ...
          {                                                                                        \
            case 0:                                                                                \
              dec_clnpass_step_macro(false, true, _flags, flagsp, flags_stride, data, l_w, 0, 0,   \
                                     vsc);                                                         \
              partial = false;                                                                     \
              /* FALLTHRU */                                                                       \
            case 1:                                                                                \
              dec_clnpass_step_macro(false, partial, _flags, flagsp, flags_stride, data, l_w, 1,   \
                                     3, false);                                                    \
              partial = false;                                                                     \
              /* FALLTHRU */                                                                       \
            case 2:                                                                                \
              dec_clnpass_step_macro(false, partial, _flags, flagsp, flags_stride, data, l_w, 2,   \
                                     6, false);                                                    \
              partial = false;                                                                     \
              /* FALLTHRU */                                                                       \
            case 3:                                                                                \
              dec_clnpass_step_macro(false, partial, _flags, flagsp, flags_stride, data, l_w, 3,   \
                                     9, false);                                                    \
              break;                                                                               \
          }                                                                                        \
        }                                                                                          \
        ...
        {                                                                                          \
          dec_clnpass_step_macro(true, false, _flags, flagsp, flags_stride, data, l_w, 0, 0, vsc); \
    ...
    {                                                                                          \
      ...
      {                                                                                        \
        ...
        {                                                                                      \
          ...
          dec_sigpass_step_mqc_macro(_flags, flagsp, flags_stride, dataPtr, l_w, 1, 3, false); \
          dec_sigpass_step_mqc_macro(_flags, flagsp, flags_stride, dataPtr, l_w, 2, 6, false); \
          dec_sigpass_step_mqc_macro(_flags, flagsp, flags_stride, dataPtr, l_w, 3, 9, false); \
          *flagsp = _flags;                                                                    \
        }                                                                                      \
      }                                                                                        \
    }                                                                                          \
  ...
  if(check_pterm)
  {
    if(mqc->bp + 2 < mqc->end)
      grk::Logger::logger_.warn(
          ...
          (int)(mqc->end - mqc->bp) - 2, (int)(mqc->bp - mqc->start), (int)(mqc->end - mqc->start));
