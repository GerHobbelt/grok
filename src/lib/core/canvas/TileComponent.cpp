      ...
      for(auto precinct : band->precincts)
      {
        ...
        for(uint32_t j = roi_grid.y0; j < roi_grid.y1; ++j)
        {
          ...
          for(uint32_t i = roi_grid.x0; i < roi_grid.x1; ++i)
          {
            ...
            {
              temp =
                  temp.rectUnion(grk_rect32(x, y, x + cblkBounds.width(), y + cblkBounds.height()));
            }
            cblkno++;
          }
        }
  ...
  for(uint8_t resno = 0; resno < numres; ++resno)
  {
    ...
    for(uint8_t bandIndex = 0; bandIndex < res->numTileBandWindows; ++bandIndex)
    {
      ...
      for(auto precinct : band->precincts)
      {
        ...
        {
          uint64_t cblkno = (roi_grid.x0 - cblk_grid.x0) + (uint64_t)(gridY - cblk_grid.y0) * w;
...
void TileComponent::postProcess(int32_t* srcData, DecompressBlockExec* block)
{
  ...
  {
    if(block->qmfbid == 1)
      postDecompressImpl<RoiShiftFilter<int32_t>>(srcData, block, (uint16_t)block->cblk->width());
    else
      postDecompressImpl<RoiScaleFilter<int32_t>>(srcData, block, (uint16_t)block->cblk->width());
  }
  else
  {
    if(block->qmfbid == 1)
