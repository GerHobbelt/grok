                                       ...
                                       const char* separator)
{
  ...
  for(fileno = 0; fileno < nbFilenamePGX; fileno++)
  {
    ...
    auto dest_comp = dest->comps + fileno;
    memcpy(dest_comp->data, dest_data[fileno], dest_comp->h * dest_comp->stride * sizeof(int32_t));
    free(dest_data[fileno]);
    dest_data[fileno] = nullptr;
  }
...
static int parse_cmdline_cmp(int argc, char** argv, test_cmp_parameters* param)
{
  ...
  try
  {
    ...
    TCLAP::ValueArg<std::string> mseArg("m", "MSE", "Mean Square Energy", false, "", "string", cmd);
...
int GrkCompareImages::main(int argc, char** argv)
{
  ...
  for(compno = 0; compno < imageBase->numcomps; compno++)
  {
    ...
    {
      spdlog::error("sign mismatch [comp {}] ({} != {})", compno, baseComp->sgnd, testComp->sgnd);
      goto cleanup;
    }
    ...
    if(inParam.regionSet)
    {
      ...
      {
        spdlog::error("test image component width {} doesn't match region width {}", testComp->w,
                      inParam.region[2] - inParam.region[0]);
        goto cleanup;
      }
      if(testComp->h != (uint32_t)(inParam.region[3] - inParam.region[1]))
      {
        spdlog::error("test image component height {} doesn't match region height {}", testComp->h,
                      inParam.region[3] - inParam.region[1]);
        goto cleanup;
      }
    }
    else
